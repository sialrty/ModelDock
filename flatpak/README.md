...
This directory contains the Flatpak manifest (`com.modeldock.desktop`) for ModelDock, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo ModelDock-Linux.flatpak com.modeldock.desktop
```

4) Install and run:

```bash
flatpak install --user ./ModelDock-Linux.flatpak
flatpak run com.modeldock.desktop
```
