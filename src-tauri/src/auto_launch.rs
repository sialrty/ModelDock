...
/// 将 `/path/to/ModelDock.app/Contents/MacOS/ModelDock` 转换为 `/path/to/ModelDock.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/ModelDock.app/Contents/MacOS/ModelDock");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/ModelDock.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/ModelDock.app/Contents/MacOS/ModelDock");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/ModelDock.app"
            ))
        );
    }
